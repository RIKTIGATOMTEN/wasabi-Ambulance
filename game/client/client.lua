...
AddEventHandler('wasabi_bridge:onPlayerDeath', function(data)
    ...
    if data.deathCause == 0 and data.victimCoords then
    ...
    for k, v in pairs(Config.Locations) do
                    ...
                    while true do
                        ...
                        if hasJob then
                            ...
                            if dist <= v.BossMenu.Distance then
                                ...
                                sleep = 0
                                if IsControlJustReleased(0, 38) then
                                    TriggerEvent("unrp_jobpanel:openJobPanel", "ambulance")
                                end
                            else
                                if textUI then
                                    wsb.hideTextUI()
        ...
        if v.Vehicles.Enabled then
            CreateThread(function()
                ...
                if v.Helicopters.Enabled then
            CreateThread(function()
                local zone = v.Helicopters.Zone
                local textUI
                while true do
                    local sleep = 1500
                    local hasJob, _grade = wsb.hasGroup(Config.ambulanceJobs or Config.ambulanceJob)
                    ...
                    if hasJob then
                        ...
                        local coords = GetEntityCoords(playerPed)
                        local dist = #(vector3(coords.x, coords.y, coords.z) - vector3(zone.coords.x, zone.coords.y, zone.coords.z))
                        local dist2 = #(vector3(coords.x, coords.y, coords.z) - vector3(v.Helicopters.Spawn.air.coords.x, v.Helicopters.Spawn.air.coords.y, v.Helicopters.Spawn.air.coords.z))
                        if dist < zone.range + 1 and not inMenu and not IsPedInAnyVehicle(playerPed, false) then
                            sleep = 0
                            if not textUI then
                                wsb.showTextUI(zone.label)
...
AddEventHandler('wasabi_ambulance:spawnVehicle', function(data)
    ...
    local spawnType = 'Vehicles' -- Default to vehicles
    if Config.Locations[data.hospital].Helicopters and Config.Locations[data.hospital].Helicopters.Options[data.grade][data.model] then
        spawnType = 'Helicopters' -- Change to helicopters if the model matches helicopter options
    end
    local category = Config.Locations[data.hospital][spawnType].Options[data.grade][data.model].category
    local spawnLoc = Config.Locations[data.hospital][spawnType].Spawn[category]
    if not IsModelInCdimage(GetHashKey(model)) then
        print('Vehicle model not found: ' .. model)
    else
        local nearbyVehicles = wsb.getNearbyVehicles(vec3(spawnLoc.coords.x, spawnLoc.coords.y, spawnLoc.coords.z), 6.0, true)
...
AddEventHandler('esx_ambulancejob:heal', function(_type)
  ...
  if _type == 'alvedon' then
    if not taken then
    taken = true
    ESX.ShowNotification("Du har tagit en Alvedon, du kommer börja känna dig bättre snart")
    Citizen.SetTimeout(10000, function()
      ...
      Citizen.SetTimeout(5000, function()
        local health = GetEntityHealth(playerPed)
        local newHealth = math.min(maxHealth , math.floor(health + maxHealth/100))
        SetEntityHealth(playerPed, newHealth)
        Citizen.SetTimeout(5000, function()
          local health = GetEntityHealth(playerPed)
          local newHealth = math.min(maxHealth , math.floor(health + maxHealth/100))
                  ...
                  Citizen.SetTimeout(5000, function()
                    ...
                    Citizen.SetTimeout(5000, function()
                      ...
                      Citizen.SetTimeout(5000, function()
                        ...
                        local newHealth = math.min(maxHealth , math.floor(health + maxHealth/100))
                        SetEntityHealth(playerPed, newHealth)
                        ESX.ShowNotification("Din Alvedons verkan har tagit slut.")
                        taken = false
                      end)
                    end)
                  end)
    ...
    -- ESX.ShowNotification("Tänker du överdosera ihjäl dig eller?")
  end
  end
  if _type == 'small' then
    if not taken then
      taken = true
      ESX.ShowNotification("Du har tagit Tigerbalsam, du kommer börja känna dig bättre snart")
      Citizen.SetTimeout(10000, function()
        local health = GetEntityHealth(playerPed)
        local newHealth = math.min(maxHealth , math.floor(health + maxHealth/12))
        SetEntityHealth(playerPed, newHealth)
                    ...
                    Citizen.SetTimeout(2000, function()
                      ...
                      Citizen.SetTimeout(2000, function()
                        ...
                        Citizen.SetTimeout(2000, function()
                          ...
                          local newHealth = math.min(maxHealth , math.floor(health + maxHealth/100))
                          SetEntityHealth(playerPed, newHealth)
                          ESX.ShowNotification("Tigerbalsamens verkan har tagit slut.")
                          taken = false
                        end)
                      end)
                    end)
  ...
  if _type == 'small2' then
    if not taken then
      taken = true
      ESX.ShowNotification("Du har tagit läkemedlet, du kommer börja känna dig bättre snart")
      Citizen.SetTimeout(10000, function()
        local health = GetEntityHealth(playerPed)
        local newHealth = math.min(maxHealth , math.floor(health + maxHealth/12))
        SetEntityHealth(playerPed, newHealth)
                    ...
                    Citizen.SetTimeout(2000, function()
                      ...
                      Citizen.SetTimeout(2000, function()
                        ...
                        Citizen.SetTimeout(2000, function()
                          ...
                          local newHealth = math.min(maxHealth , math.floor(health + maxHealth/100))
                          SetEntityHealth(playerPed, newHealth)
                          ESX.ShowNotification("Läkemedlets verkan har tagit slut.")
                          taken = false
                        end)
                      end)
                    end)
